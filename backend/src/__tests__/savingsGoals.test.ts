...
describe("Savings Goals Functionality", () => {
    ...
    test("Should update an existing savings goal current saved amount", async () => {
        ...
        await user.transaction({ id: user.transactions.length + 1, type: "credit", category: "Emergency Fund", amount: 500, date: new Date() });
    ...
    test("Should throw an error when saving amount exceeds target", async () => {
        ...
        await user.addSavingsGoal(savingsGoal);
        const transaction: ITransaction = { id: user.transactions.length + 1, type: "credit", category: "Home", amount: 5000, date: new Date() };
        await expect(user.transaction(transaction)).rejects.toThrow("Saving amount exceeding target amount");
    });

    test("Should check progress towards a specific savings goal", async () => {
        const transaction1: ITransaction = { id: user.transactions.length + 1, type: "credit", category: "Health", amount: 2500, date: new Date() };
        const transaction2: ITransaction = { id: user.transactions.length + 1, type: "credit", category: "Marraige", amount: 4600, date: new Date() };
